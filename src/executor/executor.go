...
func (e *Executor) DoPaymentTrx(warehouseId, districtId int, amount float64, cWId, cDId, cId int, cLast string, hDate time.Time, badCredit string, cdatalen int) error {
	return e.DoTrxRetries(func() error {
		return e.DoPayment(warehouseId, districtId, amount, cWId, cDId, cId, cLast, hDate, badCredit, cdatalen)
	})
}

func (e *Executor) DoPayment(warehouseId, districtId int, amount float64, cWId, cDId, cId int, cLast string, hDate time.Time, badCredit string, cdatalen int) error {
...
func (e *Executor) DoNewOrder(wId, dId, cId int, oEntryD time.Time, iIds []int, iWids []int, iQtys []int) error {
	...
	for i := 0; i < len(iIds); i++ {
		...
		if iWids[i] != wId {
			err = e.db.UpdateStock((*stocks)[i].S_I_ID, iWids[1], sQuantity, (*stocks)[i].S_YTD+iQtys[i], (*stocks)[i].S_ORDER_CNT+1, (*stocks)[i].S_REMOTE_CNT+1)
