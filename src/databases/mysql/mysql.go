...
func NewMySQL(uri string, dbname string, transactions bool) (*MySQL, error) {
	...
	if strings.Contains(uri, "?") {
		uri_ = fmt.Sprintf("%s&parseTime=true", uri)
	} else {
		uri_ = fmt.Sprintf("%s?parseTime=true", uri)
	}

	db, err := sql.Open("mysql", uri_)
	if err != nil {
		return nil, err
	}
...
func (db *MySQL) InsertOne(tableName string, d interface{}) error {
	...
	_, err := db.Client.Exec(fmt.Sprintf("INSERT INTO %s (%s) VALUES (%s)", tableName, f, strings.Join(values_, ",")))
...
func (db *MySQL) GetCustomerByName(name string, warehouseId int, districtId int) (*models.Customer, error) {
	...
	for rows.Next() {
		err = rows.Scan(&customer.C_ID, &customer.C_FIRST, &customer.C_MIDDLE, &customer.C_LAST, &customer.C_BALANCE)
...
func (db *MySQL) GetOrderLines(orderId int, warehouseId int, districtId int) (*[]models.OrderLine, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&o.OL_O_ID, &o.OL_D_ID, &o.OL_W_ID, &o.OL_NUMBER, &o.OL_I_ID, &o.OL_SUPPLY_W_ID, &o.OL_DELIVERY_D, &o.OL_QUANTITY, &o.OL_AMOUNT, &o.OL_DIST_INFO)
		if err != nil {
			return nil, err
		}
...
func (db *MySQL) GetDistrict(warehouseId int, districtId int) (*models.District, error) {
	...
	err := r.Scan(&d.D_ID, &d.D_W_ID, &d.D_NAME, &d.D_STREET_1, &d.D_STREET_2, &d.D_CITY, &d.D_STATE, &d.D_ZIP, &d.D_TAX, &d.D_YTD, &d.D_NEXT_O_ID)
...
func (db *MySQL) UpdateCredit(customerId int, warehouseId int, districtId int, balance float64, data string) error {
	...
	if len(data) > 0 {
		res, err = db.exec("UPDATE CUSTOMER SET "+
			...
			"WHERE C_ID = ? AND C_W_ID = ? AND C_D_ID = ?", -1*balance, balance, 1, data, customerId, warehouseId, districtId)
	} else {
		res, err = db.exec("UPDATE CUSTOMER SET "+
			"C_BALANCE = C_BALANCE + ?, C_YTD_PAYMENT = C_YTD_PAYMENT + ?, C_PAYMENT_CNT = C_PAYMENT_CNT + ? "+
			"WHERE C_ID = ? AND C_W_ID = ? AND C_D_ID = ?", -1*balance, balance, 1, customerId, warehouseId, districtId)
	}
...
func (db *MySQL) CreateOrder(orderId int, customerId int, warehouseId int, districtId int, oCarrierId int, oOlCnt int, allLocal int, orderEntryDate time.Time, orderLine []models.OrderLine) error {
