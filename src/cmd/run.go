...
func stats(cancel context.CancelFunc, c chan tpcc.Transaction, wg *sync.WaitGroup, ttime int, ri int, output OutputType, percentile float64) {
	...
	for {
		...
		case <-ticker.C:
			...
			fmt.Printf(format, i, float64(sCnt+dCnt+oCnt+pCnt+nCnt)/float64(ri), sCnt, float64(perc(latencies[tpcc.StockLevelTrx], percentile)), dCnt, float64(perc(latencies[tpcc.DeliveryTrx], percentile)), oCnt, float64(perc(latencies[tpcc.OrderStatusTrx], percentile)), pCnt, float64(perc(latencies[tpcc.PaymentTrx], percentile)), nCnt, float64(perc(latencies[tpcc.NewOrderTrx], percentile)), failed)
